...
getFluids = function () {
    ...
    // Selectors.find({name:param}).forEach(function(selector) { // ToDo : ajouter Estate
    var allFluids = Configurations.findOne( {"estate_id":Session.get('current_estate_doc')._id} ).fluids;
    ...
    allFluids.forEach(function(item) { // ToDo : ajouter Estate
        ...
        labelList.push(
            {label:complete_fluid, value: complete_fluid_EN }
        );
    });
    return labelList;
};
...
dpe_Energy_Grade = function (value) {
    if (value<=50) { return "A" ;}
    if (value>50 && value<=90) { return "B" ;}
    if (value>90 && value<=150) { return "C" ;}
    if (value>150 && value<=230) { return "D" ;}
    if (value>230 && value<=330) { return "E" ;}
    if (value>331 && value<=450) { return "F" ;}
    if (value>451) { return "G" ;}
};

dpe_CO2_Grade = function (value) {
    if (value<=5) { return "A" ;}
    if (value>5 && value<=10) { return "B" ;}
    if (value>10 && value<=20) { return "C" ;}
    if (value>20 && value<=35) { return "D" ;}
    if (value>35 && value<=55) { return "E" ;}
    if (value>55 && value<=80) { return "F" ;}
    if (value>80) { return "G" ;}
};

buildArrayWithZeroes = function(arraySize) {
    var tmp_array = [];
    for (var i = 0; i < arraySize; i++) {tmp_array.push(0);}
    return tmp_array ;
};
